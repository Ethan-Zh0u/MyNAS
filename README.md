...
> [!IMPORTANT]
> 已发布的线上版本为 **v0.9.2**（release `20260806T075201Z`，并已发布 GitHub Release）。当前维护候选为 **v0.9.3**：它修复远端预览大块空白和删除前 Tailscale health 的建连预算，并修复 MyNAS 原件下载到 Photos 后首页出现第二张相同照片的问题。新导入的 PhotoKit identifier 与唯一远端资源组会立即写入持久关联意图；即使另一项备份/映射恢复正在运行或 App 重启也不会丢失，首页可在服务端 mapping 完成前凭完整资源角色、字节数和 SHA-256 证明合并显示，删除权限仍只接受服务端 committed mapping。无 mapping 的历史本机副本也会在完整资源唯一命中时自动补登记；多个远端 ID 同时精确命中则拒绝猜选。候选已通过 Xcode 27 beta 的 66/66 iOS 模拟器回归，尚待当前提交的树莓派重新部署、iPhone 16 Pro 验收和 GitHub 发布。MyNAS Photos 已具备 Tailscale 私有连接、手动原始资源备份、服务端预览、受账号隔离的远程浏览与精确内容关联，以及已真机验收的受限本机删除、MyNAS-only 删除、受校验原件导入/系统分享和账号隔离缓存管理。I1/I2/I3 已分别在默认关闭的许可边界内提供本机元数据、像素分析队列与 OCR 索引；所有照片搜索统一由“照片”主页的一个搜索框显示三列缩略图，“人物”栏只预留给人物识别。前台自动备份策略已实现；G2 已完成 Wi-Fi 中断、恢复并达到 MyNAS-confirmed 最终完成，服务重启、账号/卷、iCloud-only 和长队列仍未验收，不能称为完整后台备份保证。首次部署目前需要项目维护者完成；不要把 MyNAS 当作重要文件的唯一备份。
