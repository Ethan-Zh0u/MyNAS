...
> [!IMPORTANT]
> 当前部署版本为 **v0.8.2 测试版**。网页连接和硬盘接入已经提供新手向导，并显示实时读写速率和树莓派温度；MyNAS Photos 已具备 Tailscale 私有连接、手动原始资源备份、服务端预览生成管线和受账号隔离的远程读取 API。iOS 远程图库、自动备份、恢复和删除仍在开发中。“在全新树莓派上一键安装 MyNAS 服务”仍在开发中，首次部署目前需要项目维护者完成；不要把 MyNAS 当作重要文件的唯一备份。
