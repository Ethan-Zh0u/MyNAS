...
> [!IMPORTANT]
> 线上当前部署为 **v0.9.1**。网页连接和硬盘接入已经提供新手向导，并显示实时读写速率和树莓派温度；MyNAS Photos 已具备 Tailscale 私有连接、手动原始资源备份、服务端预览、受账号隔离的远程浏览与精确内容关联，以及已真机验收的受限本机删除、MyNAS-only 删除、受校验原件导入/系统分享和账号隔离缓存管理。I1 提供默认关闭、可删除的端侧元数据索引，支持类型、日期、收藏、RAW 与尺寸检索；它不读取像素、不上传中心化服务，也不包含 OCR、人物或语义搜索。v0.9.1 的 I2 提供独立、默认关闭的像素分析许可与账号隔离本地队列，但此阶段只保存标识符/源版本，不读取像素、不请求 iCloud 原件、不联网，也没有 OCR、人物或语义结果。前台自动备份策略已实现；G2 已在受控真机完成锁屏、开发终止后重新打开、低电量暂停续传以及 Wi-Fi 中断恢复至 MyNAS-confirmed 完成，但服务重启、账号/卷、iCloud-only 和长队列仍未验收，不能称为完整后台备份保证。首次部署目前需要项目维护者完成；不要把 MyNAS 当作重要文件的唯一备份。
