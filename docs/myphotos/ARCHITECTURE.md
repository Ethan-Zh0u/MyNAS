...
| 上传会话 | `owner + volume + device + local identifier + fingerprint` 隔离传输 session；fingerprint 可包含 `clientResourceID` 以区分分片协议，但逻辑原件等价性只由排序后的资源角色、字节数与 SHA-256 证明。带 `expectedAssetID` 的关联会先验证目标 owner/volume/committed 状态及完整证明，再建立 mapping；普通去重也使用该证明。启动时把旧 session 唯一约束迁移为含 volume 的约束；仅在显式后台 capability 的启动期，按七天 TTL 清理已验证的 staging 目录和对应未完成 metadata | 完全相同的历史重复项仅迁移 mapping 并标记 `sourceSuperseded`，原始数据库记录和文件保留；仍没有运行中 TTL、并发/断电故障注入或完整孤儿扫描 |
...
4. G1 只能在 active scene 内用 PhotoKit 变化、网络和电量条件触发前台队列；服务器支持设备映射读取时，映射请求必须成功后才能自动入队，失败不是空结果。它不注册 BGTask，也不使用 background URLSession。自动备份在锁屏/退出/回收后仍持续的能力，必须使用系统允许的 BGTask/background URLSession，不能把 foreground `Task` 描述为常驻后台服务。
   iCloud-only 原件也遵守这个两段边界：按账号、默认关闭的独立许可只允许 PhotoKit 在 App 可执行时把完整资源下载到受保护临时目录；资源组完成前不能创建 G2 记录。G2/BGProcessing 只能续接已经校验和暂存的文件，绝不能在后台 handler 中访问 PhotoKit、登录 Apple 账号或直连 iCloud。关闭许可时的 network-access-required 项目进入持久等待而非失败/自旋，且不能阻塞本机已有资源的后续自动任务。
...
9. H1 的下载在全部资源通过同源、大小和 SHA-256 校验前不得调用 PhotoKit；导入只使用公开 `PHAssetCreationRequest`，结束后清除账号隔离临时目录。若可选服务器 mapping 不可用，重复下载提示只能来自账号匹配、已完成/`committed`、asset ID 匹配且本机 PhotoKit 源版本仍相同的持久化 backup proof，并按该 proof 的 `localIdentifier` 直接查询当前 PhotoKit，不能受分页时间线影响；不可由文件名、日期、尺寸或缩略图推断。全量 Photos 元数据只能产生核验候选，绝不能直接产生绿色已存在标记；存在一个当前 server-committed 锚点时，协调器必须在前台后台串行完成全部候选的资源角色、字节数和 SHA-256 核验，并自动把匹配副本送入正常 MyNAS duplicate-session 登记，用户无需逐项点按。核验不得与现有上传或映射恢复并发，也永不自动删除 Photos 项目；只有用户明确要另建物理副本时才需要二次确认。新导入项目必须尽快以 creation placeholder 返还的 local identifier 登记当前设备 mapping，避免下次显示为无关的“需要备份”项目。持久化的精确远端目标必须作为 `expectedAssetID` 发送；服务端在 mapping 前验证目标属于相同 owner/volume、已经 committed，且资源角色、字节数与 SHA-256 完全一致。传输层 client ID、文件名和 MIME/UTI 拼写不得作为内容身份；历史完全相同的重复 asset 只把 mapping 收敛到该目标并保留所有原件。H1 的直接删除与 H0 配对删除是不同 endpoint：删除仅 MyNAS 时必须以当前 gallery version、owner、非共享映射与非 processing worker 为边界，成功后只能失效本机 backup proof，不能请求 PhotoKit 删除。若用户选择图库“双重删除”，必须先由 PhotoKit 确认精确 mapping 的本机项目已移入“最近删除”，才可调用 H0；H0 拒绝时不得把远端标为删除成功，且要说明本机项目可在系统“最近删除”恢复。
