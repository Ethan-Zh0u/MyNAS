...
4. **不能宣称通用跨端删除、MyNAS 回收站或完整隐私 AI。** H0–H2 已交付受限删除、经完整性校验的导入/分享和账号隔离缓存管理；仍没有 MyNAS 回收站或静默跨端删除。阶段 I 的 I1 只有默认关闭的本地元数据索引，不包含 OCR、embedding、人物/物体分类、跨设备索引或任何身份识别。
