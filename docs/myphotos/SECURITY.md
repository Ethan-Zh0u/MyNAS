...
| 错误合并或重复导入本机原件 | 元数据只缩小候选范围；导入路径以 PhotoKit 创建占位符的 local identifier 绑定已逐资源校验的 MyNAS ID，目标持久化后即使协调器忙碌或 App 重启也不会丢失。客户端重试会把该 `expectedAssetID` 送入上传会话，服务端必须在建立 mapping 前证明目标属于同一 owner/volume、已经 committed，且资源角色、字节数与 SHA-256 完全一致；`clientResourceID`、文件名和 MIME/UTI 拼写不是内容身份。历史完全相同的重复 asset 只把 mapping 收敛到用户已验证的目标，并把重复项标记为 `sourceSuperseded`；数据库资源记录和物理原件均保留。扫描使用 `isNetworkAccessAllowed=false`，不会静默拉取 iCloud 原件；永不自动删除 Apple Photos 项目 | 后端等价证明、错误目标拒绝、历史重复收敛回归，以及 Xcode 27 beta iPhone 17 Pro 模拟器完整 71/71 均通过；旧候选 `56a3b57` 已因真机返回另一 asset ID 的完整性失败而否决，新候选仍待树莓派重新部署与 iPhone 16 Pro 重试验收 |
...
| 自动备份意外拉取 iCloud 整库 | iCloud 原件网络读取使用独立的按账号许可，默认关闭且不继承总自动开关；旧策略缺字段时解码为 false。关闭时自动导出设置 `isNetworkAccessAllowed=false`，只有 PhotoKit 的公开 network-access-required 错误进入非失败等待；手动备份仍是显式动作。开启后仅 iPhone PhotoKit 可下载，MyNAS 不持有 Apple 凭据、不直连 iCloud，且流程不调用 Photos 删除 | 2026-08-11 新增默认/手动/自动许可、旧 JSON 迁移、开启状态持久化和精确 PhotoKit 3164 分类回归；Xcode 27 beta iPhone 17 Pro 模拟器当前工作区 71/71、generic iPhoneOS arm64 构建及 `git diff --check` 通过。签名工作区已覆盖安装并启动到 iPhone 16 Pro；只读检查确认旧总自动开关开启时，新许可仍为关闭。真实 iCloud-only 原件、下载中条件变化和 G2 最终完成仍须 iPhone 验收 |
...
> v0.9.3 删除门禁校正（2026-08-06，候选）：前述失败关闭边界不变；仅将过早的 4 秒请求／6 秒资源 probe 改为与正常图库请求相同的私有、无代理 Tailscale session（12 秒请求／30 秒资源）。这避免健康 Tailnet 正在建链时 UI 错误禁用删除，但没有把“尚未证明私网可达”变成允许，也没有放宽最终动作前重查或 MyNAS 服务端的 owner、版本、完整资源组及 worker 状态检查。

> v0.9.3 远端预览布局校正（2026-08-06，候选）：远端 JPEG 衍生图只用于显示，客户端先使用服务端返回的 preview/grid 像素尺寸，并在 ImageIO 方向转换、缩放和 RGBA 解码完成后再以实际位图尺寸设置展示比例。该调整不改变原件、衍生文件、缓存命名空间、网络权限或内容校验；尺寸缺失/异常时仍使用受界限的回退比例，避免布局失控。

> v0.9.3 精确本机唯一性校正（2026-08-11，候选）：真机证据表明旧的上传 manifest fingerprint 把 `clientResourceID` 纳入身份，导致同一资源角色、116,983 字节和相同 SHA-256 的测试图因 fixture/iOS 的传输标识不同而产生两个 asset；客户端随后因返回 ID 不等于 `pendingVerifiedRemoteAssetID` 正确报出完整性失败。继任候选在会话请求中发送 `expectedAssetID`，服务端在任何 mapping 前按 owner/volume/committed 状态和完整资源证明校验，错误目标返回 422；普通去重也使用同一证明，而不把 `clientResourceID`、文件名或 MIME/UTI 拼写当作内容身份。完全相同的历史重复项只迁移 mapping 并标记 `sourceSuperseded`，数据库记录和物理原件均保留。后端回归与 71/71 模拟器测试已通过，仍待新提交重新部署、同提交签名安装及 iPhone 16 Pro 重试验收。
...
4. **不能宣称通用跨端删除、MyNAS 回收站或完整隐私 AI。** H0–H2 已交付受限删除、经完整性校验的导入/分享和账号隔离缓存管理；仍没有 MyNAS 回收站或静默跨端删除。阶段 I 的 I1 是默认关闭的本地元数据索引，I2 是默认关闭的父许可和无像素队列，I3 候选仅在双重许可后建立可删除的本地 OCR 文字索引；仍没有 embedding、人物/物体分类、跨设备索引或任何身份识别。
