...
| 错误合并或重复导入本机原件 | 元数据只缩小候选范围；导入路径以 PhotoKit 创建占位符的 local identifier 绑定已逐资源校验的 MyNAS ID，目标持久化后即使协调器忙碌或 App 重启也不会丢失。历史无 mapping 副本可串行比对全部资源角色、字节数和 SHA-256；只有唯一远端 ID 命中才进入正常 MyNAS duplicate-session，多个 ID 命中失败关闭。待登记 proof 只允许统一时间线显示合并；删除和 completed 状态仍要求服务端 committed mapping。扫描使用 `isNetworkAccessAllowed=false`，不会静默拉取 iCloud 原件；永不自动删除 Apple Photos 项目 | Xcode 27 beta generic test build 与 iPhone 17 Pro 模拟器完整 66 项通过，包含唯一/歧义匹配、待登记显示合并及旧队列兼容；iPhone 16 Pro 的现有孤儿副本修复和再次下载仍待本候选真机验收 |
...
> v0.9.3 精确本机唯一性校正（2026-08-06，候选）：PhotoKit 导入后的创建 identifier 最多等待 15 秒可读；完整 MyNAS 下载证明会立即写入 `pendingVerifiedRemoteAssetID`，忙碌时排队、重启后从全量值型元数据恢复，服务器 duplicate outcome 必须返回同一 asset ID。统一时间线可据此在 mapping 完成前先合并显示，但 `hasCurrentVerifiedBackup` 与删除候选完全不接受该字段。对历史无 mapping 副本，自动修复无需旧 mapping 锚点，但必须完整资源唯一命中；两个远端 ID 具有同一 proof 时不建立任何关联。66/66 模拟器回归已通过，真机结果尚待用户确认。
