...
4. **不能宣称通用跨端删除、MyNAS 回收站或完整隐私 AI。** H0–H2 已交付受限删除、经完整性校验的导入/分享和账号隔离缓存管理；仍没有 MyNAS 回收站或静默跨端删除。阶段 I 的 I1 是默认关闭的本地元数据索引，I2 是默认关闭的父许可和无像素队列，I3 仅在双重许可后建立可删除的本地 OCR 文字索引；仍没有 embedding、人物/物体分类、跨设备索引或任何身份识别。
