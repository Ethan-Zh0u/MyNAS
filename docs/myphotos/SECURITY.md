...
> I3 本地 OCR 文字索引边界（v0.9.2 候选，2026-08-06）：I2 的父许可是必要条件，但用户还必须明确允许 OCR 文字索引；许可缺失、父许可撤回、父许可损坏恢复和当前账号缓存清理均会拒绝或删除 OCR 数据。只有人物页的用户操作才会把当前 Photos 权限范围的静态图片交给 Apple Vision；`PhotoLibraryClient` 拒绝视频和 Live Photo，使用 2,048 px 上限的 rendered image，`isNetworkAccessAllowed=false`，所以绝不请求 iCloud 原件。Vision actor 串行运行，索引 actor 不接收图片、`PHAsset`、缩略图、文件 URL 或 Vision observation，只保存 `assetID`、metadata-derived `sourceVersion`、processor revision、OCR 原文与时间。受保护原子 JSON 位于当前 `AppCache/<server>/<user>/analysis-queue`，绑定 schema、OCR consent revision 和 `accountID + serverID + userID`；错账号复制、损坏、重复 ID、未知 schema/许可 revision、超长文字和符号链接均失败关闭。单独关闭 OCR 只删除文字索引；关闭 I2 会删除整个父许可命名空间，因此 OCR 数据不会残留。I3 不联网、不上传 MyNAS、不包含人物、物体、embedding 或语义输出。6 项 I3 回归和完整 57 项 iPhone 17 Pro 模拟器套件通过；树莓派读回、iPhone 16 Pro 验收和 GitHub Release 仍是发布前条件。
