...
- **H1 v0.8.6 发布更正（2026-08-05）：** 上述 `20260805T011724Z` 是版本源未同步时的过渡部署，不能作为当前版本事实。现已把仓库 `VERSION` 与后端 health/capabilities 的版本源一并统一为 `0.8.6`，并在经过 23 项前端测试、生产构建、Go 测试/vet 及 ARM64 编译后原子部署到 `20260805T013621Z`；systemd active，health/capabilities 均回读 `0.8.6`，`photoDelete=true`、`backgroundTransfers=true`，H1 的 `/assets/{assetID}/delete` 路由仍在。GitHub 发布将以相同的 `v0.8.6` tag 记录；这不改变“当前 iOS H1 真实媒体下载／删除尚未验收”的状态。
