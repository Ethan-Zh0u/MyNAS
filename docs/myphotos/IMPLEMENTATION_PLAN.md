...
- **串行子阶段：** I1 元数据索引收尾 → I2 端侧分析队列与独立像素分析许可 → I3 Vision OCR 与照片主页统一搜索框 → I4 系统视觉标签与相似度 → I5 MobileCLIP Core ML 选型/真机门槛 → I6 本地语义搜索 → I7 匿名人物聚类 → I8 人物集合与对应照片界面 → I9 隐私、生命周期、性能和可选 MyNAS 派生索引同步总验收。I3 先把 I1 与 OCR 接入同一个照片主页查询框；I4/I6 的内容描述、物体和语义来源只能继续接入该框，不能另建搜索入口。“人物”栏只承载 I7/I8 的人物识别、人物集合和对应照片，不承载通用搜索或 OCR 控制。任何时刻只推进一个子阶段，后续阶段不得和当前阶段并行开发或提前宣称。
