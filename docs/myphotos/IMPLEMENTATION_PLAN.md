...
| 项目 | 已核查事实 | 证据 |
| --- | --- | --- |
| 版本 | F3 精确内容关联版的 MyNAS 后端声明和仓库 `VERSION` 均为 `0.8.3`；iOS target 的 `MARKETING_VERSION` 为 `1.0`。 | `backend/photos_phase2.go`、`VERSION`、`ios/MyPhotos/MyPhotos.xcodeproj/project.pbxproj` |
...
| 已部署能力 | 树莓派 MyNAS `0.8.3` 已于 2026-07-27 原子部署；Tailscale HTTPS health 返回实时 CPU 温度，capabilities 返回状态模型 v1、`photos-browse-v1`、remote browsing/change feed、device mapping recovery、精确内容关联摘要和三项 derivative recipe。发布前快照 SQLite 为 `ok`，包含 48 个原始资源、141 个 ready 衍生文件 SHA-256 清单；生产列表/详情已确认返回匿名关联字段。 | 真实部署健康检查、数据库状态、F3 快照和 API 验收 |
...
3. **可用于保留 MyNAS 后删除本机的备份**：在第 2 项之外，恢复/导出、回收站和删除确认策略均已验收。H0 仅实现“本机进入 iOS 最近删除 + 可选 MyNAS 专用回收站”的受限配对流程，未部署、未验收前不得把它宣传为完整阶段 H 能力。
...
| F | 本地/远程统一时间线与去重 | 进行中（F1–F3 已发布；F4 显式版本转移工作区实现） | 验收版本转移、跨设备关联与 Live Photo 资源组关系 |
| G | 后台自动备份 | 未开始 | 依赖 D 的稳定恢复语义及 iOS 后台策略 |
| H | 恢复、导出、删除与缓存管理 | H0 工作区实现（未部署） | 受限的本机删除 + MyNAS Photos 回收站配对；完整恢复/导出/缓存管理仍待开始 |
...
- **数据模型/API 变化：** 依序使用 `localIdentifier + device mapping`、asset ID、manifest/资源 SHA-256、时间/尺寸和 Live Photo 资源组；文件名只能作显示信息，不能作唯一依据。F3 只返回 `exactContentDeviceCount` 和 `exactContentMappingCount` 聚合值，不返回 fingerprint、其他设备 ID 或 other-device local identifier。F4 待发布字段 `previousVersionCount` 和 `nextVersionCount` 只反映已记录的转移数量，不公开转移中的设备或本地标识，也不根据历史数据推断关系。
...
- **状态与证据：** **进行中（F3 已发布，跨设备实机验收待完成）。** `UnifiedPhotoTimelineItem` 只在当前设备的持久化备份任务仍指向当前 `PHAsset` 源版本、且含服务器确认的 MyNAS asset ID 时合并本机与远端记录；绝不以拍摄时间、文件名或缩略图猜测为同一项目。`UnifiedPhotoTimelineViewModel` 以 120 项远端 cursor 页读取并在主照片页按拍摄时间排序，远端独有项目保留“仅 MyNAS”，本机项目明确显示等待、上传、失败、原件已安全入库或“已备份 · 可浏览”。F2 服务器在 0.8.2 新增 owner + device 双重隔离的映射分页接口；iOS 以 Keychain 保留随机设备 ID，仅在本地 `PHAsset` 的源版本与服务器 `sourceModificationDate` 严格相等、且 `sourceCommitted` 时恢复完成状态，不能匹配的项目仍走正常备份。F3 在 0.8.3 以同一 owner、同一卷、完整 manifest SHA-256/角色/字节数一致的资源组返回匿名 `exactContentDeviceCount`/`exactContentMappingCount`；详情页和统一时间线只做说明，绝不静默合并或隐藏项目。服务端测试覆盖跨设备严格去重、列表/详情摘要与无 fingerprint/device ID 泄露；发布后真实列表/详情验收已确认字段和无敏感标识泄露，iOS 对字段缺失保持兼容。iPhone 17 Pro 最新构建已通过，但此前模拟器 Photos 服务 `assetsd` 退出，待其恢复后仍需完成“普通卸载重装后恢复映射”和跨设备关联的端到端回归。编辑版本/Live Photo 组的跨端关系、基于统一 cursor 的整页稳定合并，以及专门的 F 自动化测试仍待完成；删除与导出仍不在 F 范围。

**F4 当前工作区状态：** 同一设备、同一 `localIdentifier` 切到另一完整资源组时，后端会保存私有转移审计，并让列表/详情返回匿名前序/后续计数；Live Photo 测试确认静态图与 paired video 必须一起完成才形成转移。iOS 对未发布字段保持兼容，Xcode 最新构建通过。F4 尚未部署，仍需真实验证编辑、撤销编辑、跨设备相同内容与 App 重装。
...
- **用户可见成果：** H0 工作区在“本机”多选模式提供删除入口：默认只请求 iOS 将项目移入“最近删除”；可选“同时移入 MyNAS 回收站”默认关闭，且只在每项有当前、sourceCommitted 的本机→MyNAS 映射时可选。服务端会再次检查版本、完整资源组、唯一映射和衍生 worker 状态；共享原件、旧版本或不完整备份被拒绝。本机删除被系统拒绝时，应用自动请求 MyNAS 恢复刚移动的项目。预览/原件下载或导出、网页回收站、缓存空间与 LRU 清理仍待实现。
...
- **MyNAS 后端改动：** H0 新增 owner-scoped `photo_trash_entries` 和完整资源组的软删除/恢复。原件目录与存在的衍生目录同卷移动到 `.mynas/photos-trash/<opaque id>`；活动图库、资源流、映射恢复和 worker 都排除 `trashed` source state。不能复用无 owner 语义的通用文件删除流程。
- **数据模型/API 变化：** 新增 `photo_trash_entries`、asset `source_state=trashed`、批量 `/assets` trash 和 `/assets/restore`；删除 intent 只能携带本机映射证明，fingerprint 与存储路径永不返回客户端。保留期与可视回收站 API 待设计。
...
## 下一步：先将 F4 与 H0 作为独立受控发布候选，再验收版本转移与删除恢复

阶段 E 已完成，F1/F2/F3 已提供不会误合并的只读时间线、同设备映射恢复和跨设备内容关联。F4 的下一道门槛是 **在同一设备编辑或撤销编辑后，保留每一份原件并只展示已记录的版本转移，不从外观推断关系**：

1. 以同一测试设备备份普通照片与 Live Photo，编辑后再次备份，再撤销编辑并备份；确认原件均保留，且只出现由同一 `localIdentifier` 切换产生的匿名转移计数。
2. 以第二台受控设备备份字节完全相同的普通照片、Live Photo 资源组和 ProRAW；确认它只增加 F3 内容关联，不能制造 F4 版本转移。
3. 验证相同文件名、相同/相近拍摄时间但任一资源 SHA-256 或字节数不同的媒体绝不建立关联。
4. 用本机独有、远端独有、同内容不同设备、App 重装、编辑后/撤销编辑及 Live Photo 组做真机/模拟器回归。删除、导出和后台自动备份仍分别留在 H、G 阶段。
