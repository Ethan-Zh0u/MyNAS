...
retry "remote preflight" ssh "${ssh_options[@]}" "$remote" \
  "set -eu; sudo -n true; command -v ffmpeg; command -v simple_dcraw; command -v heif-convert; findmnt -no SOURCE,FSTYPE,TARGET /mnt/nas; df -h /mnt/nas; systemctl is-active --quiet smbd; mkdir -p '$remote_release/web'"
